  ...
  <div class="flex justify-center w-full">
    <div class="flex flex-col gap-6 items-start w-full">
        ...
        <ClientOnly>
          <u-form-field>
            ...
            <template #description>
              <p>
                In this series of tutorials, we implemented a blog from scratch. We added cool
                features such as responsive Table of Content with microinteractions and made the
                blog SEO ready. <br />
              </p>
            </template>
          </u-form-field>
      ...
      <section class="articles-grid">
        <ArticleCard v-for="article in filteredArticles" :article="article" class="aspect-[2/1]">
        </ArticleCard>
      </section>
    </div>
    <Modal
          ...
          <li v-for="link of result" :key="link.item.id" class="mt-2">
            <u-button variant="ghost" color="neutral" class="w-full" :to="link.item.id">
              <div class="flex flex-col w-full">
                {{ link.item.title }}
                <span
                  class="typ-sublabel block w-full whitespace-nowrap overflow-hidden text-ellipsis"
                >
                  {{ link.item.content }}...
                </span>
              </div>
